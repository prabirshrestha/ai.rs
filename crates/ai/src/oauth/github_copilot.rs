...
// Generated by Pi from the models.dev GitHub Copilot catalog. Keep this list in
// sync with Pi's GITHUB_COPILOT_MODELS because policy enablement intentionally
// targets known catalog models rather than every raw /models response entry.
const GITHUB_COPILOT_MODEL_IDS: &[&str] = &[
    "claude-fable-5",
    "claude-haiku-4.5",
    "claude-opus-4.5",
    "claude-opus-4.6",
...
async fn enable_copilot_models(
    ...
    let base_url = get_github_copilot_base_url(Some(copilot_token), enterprise_domain);
    let client = reqwest::Client::new();
    join_all(model_ids.iter().copied().map(|model_id| {
